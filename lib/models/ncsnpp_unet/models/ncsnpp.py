...
from models.ncsnpp_unet.models import layers, layerspp, normalization
...
class NCSNpp(nn.Module):
    ...
    def __init__(self, model_config):
        ...
        self.act = act = get_act(model_config)

        self.nf = nf = model_config.nf
        ch_mult = model_config.ch_mult
        self.num_res_blocks = num_res_blocks = model_config.num_res_blocks
        self.attn_resolutions = attn_resolutions = model_config.attn_resolutions
        dropout = model_config.dropout
        resamp_with_conv = model_config.resamp_with_conv
        self.num_resolutions = num_resolutions = len(ch_mult)
        self.all_resolutions = all_resolutions = [
            model_config.resolution // (2**i) for i in range(num_resolutions)
        ]

        self.conditional = conditional = model_config.conditional  # noise-conditional
        fir = model_config.fir
        fir_kernel = model_config.fir_kernel
        self.skip_rescale = skip_rescale = model_config.skip_rescale
        self.resblock_type = resblock_type = model_config.resblock_type.lower()
        self.progressive = progressive = model_config.progressive.lower()
        self.progressive_input = (
            progressive_input
        ) = model_config.progressive_input.lower()
        self.embedding_type = embedding_type = model_config.embedding_type.lower()
        init_scale = model_config.init_scale
        assert progressive in ["none", "output_skip", "residual"]
        assert progressive_input in ["none", "input_skip", "residual"]
        assert embedding_type in ["fourier", "positional"]
        combine_method = model_config.progressive_combine.lower()
